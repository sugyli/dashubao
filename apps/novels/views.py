...
class M_IndexView(View):
    ...
    def get(self, request):
        all_noveldetail = modelhelp.get_all_book().order_by("-create_time")
        try:
...
class M_InfoView(View):
    def get(self, request, bookid):
        ...
        if noveldetail:
            ...
            if all_chapter:
                first_chapter_addr = noveldetail.get_first_chapter(all_chapter,isdaoxu=isdaoxu)
                try:
                    page = request.GET.get('page', 1)
                except PageNotAnInteger:
                    page = 1
