...
try:
    ...
    # 使用execute方法执行SQL语句
    cursor3.execute(
        "select * from novels_noveldetail where have_chapter = 1 and ishide = 0   order by id asc limit 1")
    ...
    while True:
        ...
        if not data:
            ...
            for item in data:
                url_md5 = item[2]
                print('查询到id %s 书名 %s 的数据 准备替换' % (item[0], item[3]))
                cursor.execute(
                    "select * from novels_novelchapter where noveldetail_id =%s and chapter_type = 0  order by chapter_order asc"%url_md5)
                count = 1
                while True:
